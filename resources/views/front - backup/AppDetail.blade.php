...
@include('front.leftnav')
					...
					<tr><td>ภาษาที่ใช้พัฒนา</td><td>{{ $modell->develop_language }}</td></tr>
					<tr><td>ฐานข้อมูล</td><td>{{ $modell->app_database }}</td></tr>
					<tr><td>บริษัทที่พัฒนา</td><td>{{ $modell->develop_company }}</td></tr>
					<tr><td>ปีที่เริ่มใช้งาน</td><td>{{ $modell->getting_start_years }}</td></tr>
					<tr><td>แอพพลิเคชั่นที่เกี่ยวข้อง</td><td>
						<?php $i=1; ?>
